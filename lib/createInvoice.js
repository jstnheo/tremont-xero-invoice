...
async function createInvoice(contact, reservation) {
  ...
  // Get property tracking name (e.g., "Unit 2")
  const propertyName = properties?.[0]?.name || null;
  const trackingOption = propertyName ? { Tracking: [{ Name: 'Property', Option: propertyName }] } : {};
  ...
  // 3. Hosting Fee
  const hostServiceFee = financials?.host?.host_fees?.find(f => f.label.toLowerCase().includes("host service"));
  if (hostServiceFee) {
    lineItems.push({
      Description: `${platform === 'vrbo' ? 'VRBO' : 'Airbnb'} Hosting Fee`,
      Quantity: 1,
      UnitAmount: (Math.abs(hostServiceFee.amount) / 100 * -1).toFixed(2), // force negative
